...
分类:
  - "网页裁剪"
标题: "Quantifiers (The Java™ Tutorials >        
...
[[Java核心类库-正则表达式-预定义字符类|« Previous]] • [Trail](https://docs.oracle.com/javase/tutorial/essential/TOC.html) • [[Java核心类库-正则表达式-捕获组|Next »]]
...
Until now, we've only tested quantifiers on input strings containing one character. In fact, quantifiers can only attach to one character at a time, so the regular expression "abc+" would mean "a, followed by b, followed by c one or more times". It would not mean "abc" one or more times. However, quantifiers can also attach to [[Java核心类库-正则表达式-字符类|Character Classes]] and [[Java核心类库-正则表达式-捕获组|Capturing Groups]], such as `[abc]+` (a or b or c, one or more times) or `(abc)+` (the group "abc", one or more times).
