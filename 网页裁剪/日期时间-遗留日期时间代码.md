...
---
# Legacy Date-Time Code (The Java™ Tutorials >        
            Date Time > Standard Calendar)

Documentation

[[日期时间-ISO标准-nonIso|« Previous]] • [Trail](https://docs.oracle.com/javase/tutorial/datetime/TOC.html) • [[日期时间-ISO标准-summary|Next »]]
...
The table introduced on the [[日期时间-ISO标准-overview|Overview]] page is a good place to begin evaluating which java.time classes meet your needs.
...
| java.util.TimeZone | java.time.ZoneId or java.time.ZoneOffset | The ZoneId class specifies a time zone identifier and has access to the rules used each time zone. The ZoneOffset class specifies only an offset from Greenwich/UTC. For more information, see [[日期时间-ISO标准-timezones|Time Zone and Offset Classes]]. |
