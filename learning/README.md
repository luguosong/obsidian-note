...
- **滚动条**：侧栏 nav / 右侧 TOC / 代码块 `pre` 统一用**细窄暖调滚动条**（宽 8px，静止 `--rule` 极淡、悬停 panel 转 `--faint`、悬停 thumb 转 `--accent` 金；透明边框 + `background-clip:padding-box` 自适应不同底色），避免默认粗亮滚动条破坏暗色。写在 `style.css`，新组件若自带滚动追加选择器即可。2026-07-31 起。
- **图示缩放**：凡含 mermaid 图的课，`<head>` 引 `assets/diagram-zoom.js`（新模板 `lesson-template.html` 已默认引入）；每张 `figure.diagram` 自动获得右上「⛶ 放大」→ 全屏浮层（滚轮对准光标缩放 / 拖拽平移 / −·+·1:1 按钮 / Esc·✕·点背景 关闭）。样式在 `style.css`「图示缩放」段，纯原生无依赖、打印自动隐藏。**新建课程复制 assets 时一并带上 `diagram-zoom.js`**，图片默认即可全屏缩放，无需逐课接线。2026-08-13 起。

来源：2026-07-20 用户反馈"文字不够醒目"，把正文从 ≈7:1 提到 **≈13:1**；2026-07-21 应用户要求两门课整体切为「墨夜」暗色（高对比与护眼不变）；2026-07-31 应用户反馈"侧栏滚动条不美观"，加细窄暖调滚动条并固化为统一约定；2026-08-13 应用户要求把「图片放大缩放」固化为默认能力（`diagram-zoom.js`），以后新课程的图默认可全屏缩放。以后所有课程网页都用这套。
