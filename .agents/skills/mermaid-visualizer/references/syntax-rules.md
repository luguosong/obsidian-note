...
```mermaid
WRONG: Title --> Core Process      # Cannot reference display name
RIGHT: Title --> core              # Must reference ID
```
...
# Reference nodes
A --> B                        RIGHT: Use node IDs
Display Text A --> Display Text B  WRONG: Cannot use display text
```
...
RIGHT: `1.Text` `①Text` `(1)Text` `Step 1:Text`
WRONG: `1. Text`

### Safe Subgraph Syntax
RIGHT: `subgraph id["Name"]` `subgraph simple_name`
WRONG: `subgraph Name With Spaces`

### Safe Node References
RIGHT: `NodeID --> AnotherID`
WRONG: `"Display Text" --> "Other Text"`

### Safe Special Characters
RIGHT: `『』` for quotes, `「」` for parentheses
WRONG: `"` unescaped quotes, `()` in problematic contexts
